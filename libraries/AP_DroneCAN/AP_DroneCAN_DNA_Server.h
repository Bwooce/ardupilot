...
public:
    ...
    // Check if a node has been seen (for PARAM_EXT DroneCAN bridge)
    bool is_node_seen(uint8_t node_id) { return node_seen.get(node_id); }

    // Request re-verification of all seen nodes (for MAV_CMD_UAVCAN_GET_NODE_INFO)
    ...
    // ESP32-specific accessors for LED display
