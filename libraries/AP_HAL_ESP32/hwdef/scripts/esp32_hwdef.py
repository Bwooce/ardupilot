...
class ESP32HWDef(hwdef.HWDef):
    ...
    # This allows easy addition of new chips (C6, H2, P4) by defining their constraints here
    CHIP_DATA = {
        'ESP32': {
            'reserved': {6, 7, 8, 9, 10, 11}, # SPI Flash
        ...
        'ESP32S2': {
            'reserved': {26, 27, 28, 29, 30, 31, 32}, # Flash/PSRAM
            'adc': set(range(1, 21)),
            'input_only': {46},
            'strapping': {0, 45, 46},
        ...
        'ESP32S3': {
            'reserved': {26, 27, 28, 29, 30, 31, 32}, # OPI Flash/PSRAM
            'adc': set(range(1, 21)), # ADC1 & ADC2
            'input_only': set(),
            'strapping': {0, 3, 45, 46},
            'features': ['HAL_ESP32_HAS_MCPWM', 'HAL_ESP32_HAS_USB_SERIAL_JTAG', 'HAL_ESP32_LARGE_BUFFERS']
        },
        'ESP32C3': {
            'reserved': {12, 13, 14, 15, 16, 17}, # SPI Flash
            'adc': {0, 1, 2, 3, 4, 5},
            'input_only': set(),
            'strapping': {2, 8, 9},
        ...
        'ESP32C6': {
            'reserved': set(), # Fill when validating
            'adc': set(),
            'input_only': set(),
            'strapping': set(),
            'features': ['HAL_ESP32_HAS_USB_SERIAL_JTAG', 'HAL_ESP32_HAS_MCPWM'] # C6 has MCPWM
        },
        'ESP32P4': {
            'reserved': set(), # Fill when validating
            'adc': set(),
            'input_only': set(),
            'strapping': set(),
            'features': ['HAL_ESP32_HAS_MCPWM', 'HAL_ESP32_HAS_MIPI', 'HAL_ESP32_HAS_USB_SERIAL_JTAG', 'HAL_ESP32_LARGE_BUFFERS'] # P4 has no radio
        }
    }
    ...
    def init_hardware_constraints(self):
        ...
        if chip:
            ...
            self.progress(f"{self.mcu.upper()} constraints loaded: ADC({len(self.adc_capable_pins)}), Reserved({len(self.reserved_pins)})")
        else:
    ...
    def process_line(self, line, depth=0):
        ...
        elif line.startswith("RESERVED_PINS"):
            parts = line.split()
            if len(parts) > 1:
                # Add all listed pins to reserved set
                for pin in parts[1:]:
                    try:
                        self.reserved_pins.add(int(pin))
                    except ValueError:
                        self.error(f"Invalid pin number in RESERVED_PINS: {pin}")
                self.progress(f"Added user-defined reserved pins: {parts[1:]}")
    ...
    def generate_hwdef_h(self):
        ...
        # Auto-generated feature defines from CHIP_DATA
        chip = self.CHIP_DATA.get(self.mcu.upper())
        if chip and 'features' in chip:
