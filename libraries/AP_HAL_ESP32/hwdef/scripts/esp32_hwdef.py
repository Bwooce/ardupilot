    ...
    def write_esp32_config(self, f):
        ...
        if self.spi_buses:
            ...
            for b in sorted(self.spi_buses, key=lambda x: x['num']):
                ...
                entries.append(f"    {{.host={host}, .dma_ch=1, .mosi=GPIO_NUM_{b['mosi']}, "
                               f".miso=GPIO_NUM_{b['miso']}, .sclk=GPIO_NUM_{b['sck']}}}")
            f.write(',\\\n'.join(entries) + '\n#endif\n')
        ...
        if self.spi_devices:
            ...
            for i, d in enumerate(self.spi_devices):
                entries.append(f'    {{.name="{d["name"]}", .bus=0, .device={i}, '
                               f'.cs=GPIO_NUM_{d["cs"]}, .mode=0, .lspeed=1*MHZ, .hspeed=10*MHZ}}')
            f.write(',\\\n'.join(entries) + '\n#endif\n')

        if self.i2c_buses:
            f.write("\n/* I2C Configuration */\n#ifndef HAL_ESP32_I2C_BUSES\n"
                    "#define HAL_ESP32_I2C_BUSES \\\n")
            entries = [f"    {{.port=I2C_NUM_{b['num']}, .sda=GPIO_NUM_{b['sda']}, "
                       f".scl=GPIO_NUM_{b['scl']}, .speed=400*KHZ, .internal=true}}"
                       for b in sorted(self.i2c_buses, key=lambda x: x['num'])]
            f.write(',\\\n'.join(entries) + '\n#endif\n')
